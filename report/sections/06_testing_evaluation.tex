...
Accessibility cannot be reduced to running one tool and reading a pass/fail number. Automated
checkers, by W3C's own admission, catch only a fraction of real barriers; the rest needs human
judgement and evidence from actual tasks. Our approach is therefore \textbf{evidence triangulation}
across five layers (Figure~\ref{fig:layers}), and --- just as importantly --- a discipline of
separating \emph{raw suggestions} from \emph{confirmed defects}.
