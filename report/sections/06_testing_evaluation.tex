...
No single tool certifies accessibility: automated verifiers catch only a fraction of real barriers
(W3C's own position), so the rest needs human judgement and task evidences. We therefore
conduct the manual verification across four layers (Figure~\ref{fig:layers}).
...
TalkBack is the standard for the text-to-speech experience. Our task is to run each core flow twice:
...
and post-action focus. The common issues that the elements might have are wrong or missing labels, focus that jumps after a dialog, over-merged cards that hide a child
...
(labels, touch targets, contrast). To practise the evidence-collection workflow we captured a 15-screen flow through Agoda to fairly analyze it
...
The 15-screen capture produced 102 raw suggestions, distributed as in
Figure~\ref{fig:dist}. The highlighted result aren't equivalent to actual problems in accessiblity. Loading/skeleton states, web-view content, and the \emph{same} chip appearing across
several frames all inflate the count (report~2 and report~14 overlap; the splash screen produced
none). To access the cause of these problem, the developers should isolate the root case and re-confirm it with TalkBack or any screen reader services.
...
\begin{itemize}[leftmargin=1.4em]
	\item \textbf{Layout Inspector} shows the live merged and unmerged semantics trees, linking a
	      TalkBack/Scanner symptom to a specific node.
...
completion time, number of TalkBack gestures, automated pass rate, and a \emph{confirmed-defect
	rate} that distinguishes real bugs from raw suggestions. Findings are triaged by \textbf{their impact on
	the task} by the following order: Blocker (task impossible, fix before ship) $>$ Major $>$ Moderate
$>$ Minor. A missing label on the checkout button outweighs many
touch-target warnings on secondary content.

Every technique in this report has its trade-off, and naming them is part of an honest evaluation:
...
\begin{itemize}[leftmargin=1.4em]
	...
	\item \code{traversalIndex} fixes reading order but hand-tuned indices are brittle and can drift
	      from the visual layout; use them only when required and document the reason.
	\item Custom controls give design freedom but must redeclare role, state, and keyboard behaviour
	      that Material already provided as pre-made attributions .
\end{itemize}
