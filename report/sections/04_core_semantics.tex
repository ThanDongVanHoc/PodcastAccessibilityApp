...
Five modifiers make a singular component accessible: labelling, roles, state, touch-target size, and
scalable type. They fix the most common and most damaging screen-reader confusion.

\subsection{Content descriptions}

A screen reader cannot see an icon; it can only read the icon's text alternative. In Jetpack Compose, the
text alternative is given by \code{contentDescription} parameter. Three following rules make a good label:
...
TalkBack now announces \emph{``Pause, button, Playing''}, giving the name, role, and state, which is
...
(WCAG~2.5.5 asks for 44\,px). The visual icon can stay small; only the touchable area must
...
This is the same pattern Gmail uses for the small ``star'', or favourite, icon: a 24\,dp glyph with a 48\,dp hit
...
\begin{enumerate}[leftmargin=1.4em]
	...
	\item Never hard-code a \code{height()} on a text container; use \code{defaultMinSize} or
