...
This section covers the foundational modifiers that make an individual component accessible:
labelling, roles, state descriptions, touch-target sizing, and scalable typography. These are the
techniques that fix the most common and most damaging defects a screen-reader user encounters.

\subsection{Content descriptions: naming visual elements}

Screen readers cannot ``see'' an icon; they can only read its text alternative. In Compose this is
the \code{contentDescription} parameter. Three rules govern good labels:

\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Succinct \& action-oriented.} Avoid ``Image of'' / ``Button to'' --- the role is
...
Real apps apply exactly this split: Instagram gives its action buttons descriptive labels
(``Like'', ``Comment'') but passes \code{null} to the decorative gradients around stories, so
screen-reader users move between real content quickly.
