...
To ground the techniques in something concrete, we built \textbf{Accessible Podcast}, a small but
complete Jetpack Compose application. It is front-end only: there is no
network or database. All content is served by an in-memory mock that implements a repository
interface, so the UI code is identical to what it would be with a real backend, but the project
