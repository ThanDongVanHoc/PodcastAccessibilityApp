...
June~2022. Its technical yardstick is \textbf{WCAG~2.2}. WCAG targets the web interface designs, so for a native app we
apply it through \emph{WCAG2ICT} and mobile guidance, in which we map each success criterion to its
...
\begin{itemize}[leftmargin=1.4em,itemsep=1pt]
	...
	\item \textbf{Robust:}  expose a correct semantics tree instead of relying on position or composition tree.
\end{itemize}
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[
		...
		\node[box] (o) at (4.2,0) {\cap{O}perable\\[2pt]\small Every action is doable by any input: touch, keyboard, switch, screen reader.};
	...
	\end{tikzpicture}
	\caption{The four POUR principles that are the pillars of WCAG.}
	\label{fig:pour}
\end{figure}

\subsection{Two trees, one screen}

In Android, there is a difference between what user can see and what a screen reader  can interpret.
With Jetpack Compose, each of them are represented by a tree (Figure~\ref{fig:trees}).
...
\begin{figure}[h]
	...
	\caption{The visual tree (left) is drawn; the semantics tree (right) is read by TalkBack. The
		decorative image is dropped and the two texts are merged into one node. The techniques
		of §\ref{sec:core} and~§\ref{sec:adv}.}
	\label{fig:trees}
\end{figure}

TalkBack and Switch Access run as system services that subscribe to the merged semantics tree,
