...
\usepackage{fontspec}
% Windows-standard fonts (installed on every Windows machine, full Vietnamese support)
% so the report builds for the whole group without extra font installs.
...
\input{sections/03_casestudy}
\input{sections/04_core_semantics}
\input{sections/05_advanced_semantics}
\input{sections/06_testing_evaluation}
